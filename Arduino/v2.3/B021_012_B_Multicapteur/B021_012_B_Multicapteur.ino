/***************************************************************************************
  ...
  ou pour forcer l'inverion de sens.
...
#define WELCOME_1 "*********************************"
#define WELCOME_2 "*****  MULTI SENSOR CONTROL *****"
#define WELCOME_3 "*****         v2.3          *****"
#define WELCOME_4 "*****                       *****"
#define WELCOME_5 "*****   by Pierrick SERRES  *****"
#define WELCOME_6 "***** Copyright SUBC-MARINE *****"
#define WELCOME_7 "*********************************"

// Valeurs par défaut après programmation de la carte
#define DEFAUT_PERIODE_INVERSION 60
#define DEFAUT_DEADTIME 2
#define DEFAUT_FACTEUR_K_DEBITMETRE 7.737
#define DEFAUT_DEBIT_MIN 50
#define DEFAUT_DEBIT_MAX 300
#define DEFAUT_COURANT_MIN 1.0
#define DEFAUT_COURANT_MAX 8.0
...
/*
  ...
  [commande]: La commande. Suivant la liste de commande suivante :
            ...
            : SENS : on force l'einvcersion de sens de l'alimentation
  <LF>      : Caractère de fin de trame, 0x0A.
...
void LectureTrame(String str, int nb_values)
{
  ...
  if(str.startsWith("DATA-C")) // Demande de données
  {
    ...
    {
      GetClock();
      //sprintf(tempStr, "%02d%02d%02d,%02d%02d%02d", Date, Month, Year, Hour, Minute, Second);
 /*
      sprintf(tempStr, ",%d,%.1lf,%.1lf,%.1lf,%d,%d,%d",
        (int)debitFiltered,
        currentCommand,
        current,
        cellVoltage,
      ...
      sprintf(tempStr, ",%d,%d,%.1lf,%.3lf,%d,%d,%.1lf,%.1lf\n",
        params.val.periodeInversion - timerInversion,
        params.val.periodeInversion,
        params.val.deadtime,
...
*/
  ...
  else if(str.startsWith("CDE-C")) // Réglage de la RTC
  //SETDATEYYMMDDHHMMSS\n
  {
    if(str[5] == adresse)
    {
